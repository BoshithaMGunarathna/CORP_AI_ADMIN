...
import { customerInputItems, supplierInputItems } from "../../utils/dataArrays";
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className="w-[400px] relative h-full bg-white shadow-xl p-4"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              ...
              <div className="mt-4">
                <form onSubmit={handleSubmit}>
                  <div className="mt-6">
                    {!showNextPage &&
                      customerInputItems.slice(0, 4).map((item, index) => (
                    ...
                    {showNextPage &&
                      customerInputItems.slice(4, 8).map((item, index) => (
                        <div className="mb-3 w-full" key={index}>
                          <FormInput
                            data={formData}
                            type={item.type}
                  ...
                  <div className="absolute bottom-5 flex justify-between w-full pr-[30px]">
                    {!showNextPage ? (
                      <button
                        ...
                        className="font-poppins border-adminAddButton bg-adminAddButton flex min-w-[120px] items-center justify-center gap-2 rounded-lg border px-4 py-2 text-[14px] font-semibold leading-[22px] text-white transition-all duration-300 hover:bg-opacity-75 hover:text-white"
                      >
                        Next
                      </button>
                    ) : (
                      <>
                        <div className=" min-w-[120px]">
                          <BackButton onClick={() => setShowNextPage(false)} />
                        </div>
                        <div className=" min-w-[120px]">
                          <SubmitButton text="Save" submitting={submitting} />
                        </div>
                      </>
                    )}
                  </div>
